...
import { rememberReturnTarget, rememberPurchaseAsset } from '@/features/ton-connect';
...
// Capture a dApp's return target (`ret`) and purchase asset (`asset`) as early
// as possible: the root route redirects on mount and would otherwise strip the
// query before the `/ton-connect` route or any approval modal can read it.
rememberReturnTarget(window.location.href);
rememberPurchaseAsset(window.location.href);
