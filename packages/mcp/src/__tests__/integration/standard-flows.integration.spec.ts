...
describe.skipIf(!getIntegrationMnemonic())('MCP standard wallet flows (testnet reads, emulated sends)', () => {
    ...
    it('send_ton: captured BOC emulates with a ton_transfer action', async () => {
        const { payload, actionTypes, totalFees } = await sendEmulated(getHarness(), 'send_ton', {
    ...
    it('send_jetton: captured BOC emulates with a jetton_transfer action', async () => {
        const jetton = await fetchFixtureJetton(getHarness());
        const amountRaw = jetton.amountRaw / 10n > 0n ? jetton.amountRaw / 10n : 1n;
        const { actionTypes } = await sendEmulated(getHarness(), 'send_jetton', {
            toAddress: FIXTURES.agenticWalletAddress,
            jettonAddress: jetton.address,
            amount: formatUnits(amountRaw.toString(), jetton.decimals),
            comment: 'mcp integration test',
        });
        expect(actionTypes).toContain('jetton_transfer');
    });

    it('send_nft: captured BOC emulates with an nft_transfer action', async () => {
        const { actionTypes } = await sendEmulated(getHarness(), 'send_nft', {
            nftAddress: FIXTURES.nftItemAddress,
            toAddress: FIXTURES.agenticWalletAddress,
            comment: 'mcp integration test',
        });
        expect(actionTypes).toContain('nft_transfer');
    });

    it('send_raw_transaction: captured BOC emulates with fees', async () => {
        const { totalFees } = await sendEmulated(getHarness(), 'send_raw_transaction', {
