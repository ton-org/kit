...
function parseToolResult(
    result: Awaited<ReturnType<ReturnType<typeof createMcpTransferTools>['build_ton_transfer']['handler']>>,
) {
...
describe('transfer tools', () => {
    it('build_ton_transfer prepares a transaction without broadcasting', async () => {
        const service = {
        ...
        const result = parseToolResult(
            await tools.build_ton_transfer.handler({
                toAddress: 'EQTo',
                amount: '1.5',
            }),
        );
    ...
    it('build_jetton_transfer converts the amount with jetton decimals and prepares messages', async () => {
        const service = {
        ...
        const result = parseToolResult(
            await tools.build_jetton_transfer.handler({
                toAddress: 'EQTo',
    ...
    it('build_jetton_transfer fails when jetton decimals cannot be determined', async () => {
        const service = {
        ...
        const tools = createMcpTransferTools(service);
        const result = await tools.build_jetton_transfer.handler({
            toAddress: 'EQTo',
